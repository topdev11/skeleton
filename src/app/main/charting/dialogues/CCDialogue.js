...
function SimpleList(props) {
  ...
  const handleListItemClick = (e,value) => {
    dispatch(Actions.addListItem(value));
  }
...
function CCDialogue(){
  ...
  const redux_list_data = useSelector(({cc_dialogue}) => cc_dialogue.chartings.cc_list_data);
  ...
  useEffect(() => {
    ...
    setList_data(redux_list_data.data);
  },[redux_list_data]);
      ...
      <Dialog onClose={() => handleClose()} aria-labelledby="simple-dialog-title" open={open} fullWidth={true} maxWidth="md">
        ...
        <div>
          ...
          <TabPanel value={value} index={0}>
            <SimpleList data={list_data.data}/>
          </TabPanel>
